...
<div style="display: grid; grid-template-columns: 2fr 1fr; gap: 32px; align-items: start;">
    <div class="card">
        <div class="flex-between" style="margin-bottom: 24px; border-bottom: 1px solid var(--md-sys-color-surface-variant); padding-bottom: 16px;">
            <h2 style="font-size: 22px; color: var(--md-sys-color-primary); margin: 0;">Informations de la Demande</h2>
            <span style="font-size: 14px; color: var(--md-sys-color-outline);">Réf: BF-<?= date('Y') ?>-<?= str_pad((string)$demande['id'], 4, '0', STR_PAD_LEFT) ?></span>
        </div>

        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 24px; margin-bottom: 32px;">
            <div>
                <label style="font-size: 12px; color: var(--md-sys-color-outline); text-transform: uppercase;">Demandeur</label>
                <div style="font-weight: 500; font-size: 16px;"><?= htmlspecialchars($demande['prenom'] . ' ' . $demande['nom']) ?></div>
            </div>
            <div>
                <label style="font-size: 12px; color: var(--md-sys-color-outline); text-transform: uppercase;">Service</label>
                <div style="font-weight: 500; font-size: 16px;"><?= htmlspecialchars($demande['service_nom']) ?></div>
            </div>
            <div>
                <label style="font-size: 12px; color: var(--md-sys-color-outline); text-transform: uppercase;">Fonction</label>
                <div style="font-weight: 500; font-size: 16px;"><?= htmlspecialchars($demande['fonction']) ?></div>
            </div>
            <div>
                <label style="font-size: 12px; color: var(--md-sys-color-outline); text-transform: uppercase;">Date de création</label>
                <div style="font-weight: 500; font-size: 16px;"><?= date('d/m/Y H:i', strtotime($demande['created_at'])) ?></div>
            </div>
        </div>

        <div style="margin-bottom: 32px;">
            <label style="font-size: 12px; color: var(--md-sys-color-outline); text-transform: uppercase;">Objet de la demande</label>
        ...
        <div style="background: var(--md-sys-color-primary-container); color: var(--md-sys-color-on-primary-container); padding: 24px; border-radius: 16px; display: flex; justify-content: space-between; align-items: center;">
            <span style="font-weight: 500; font-size: 18px;">Montant Total</span>
            <span style="font-size: 28px; font-weight: 700;"><?= number_format((float)$demande['montant'], 2, ',', ' ') ?> FCFA</span>
        </div>
    </div>

    <div class="card">
        <h2 style="font-size: 18px; margin-top: 0; margin-bottom: 24px; border-bottom: 1px solid var(--md-sys-color-surface-variant); padding-bottom: 12px;">Parcours de Validation</h2>
        ...
        <div style="margin-top: 32px; padding-top: 24px; border-top: 1px solid var(--md-sys-color-surface-variant);">
            <label style="font-size: 12px; color: var(--md-sys-color-outline); text-transform: uppercase; display: block; margin-bottom: 12px;">Statut Actuel</label>
